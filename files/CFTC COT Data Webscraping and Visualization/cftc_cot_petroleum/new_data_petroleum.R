...
petroleum_page <- read_html("https://www.cftc.gov/dea/options/petroleum_lof.htm")

# Get preformatted text
petroleum_preformatted <- petroleum_page |>
  html_nodes("pre") |>
  html_text()
...
# Find Date from
new_date <- petroleum_list[[1]][[1]][2] |>
  pull() |>
  as.Date()

# Load Old Data
old_data <- readRDS("Energy Analysis/data/cftc_petroleum_historical_clean.rds")
# Combine with New
updated_data <- c(setNames(list(petroleum_list), new_date), old_data)
# Save Combined
saveRDS(updated_data, "Energy Analysis/data/cftc_petroleum_dynamically_updated.rds")
