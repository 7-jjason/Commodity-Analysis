...
ag_page <- read_html("https://www.cftc.gov/dea/options/ag_lof.htm")

# Get pre-formatted text
ag_preformatted <- ag_page |>
  html_nodes("pre") |>
  html_text()
...
# Find Date from
new_date <- ag_list[[1]][[1]][2] |>
  pull() |>
  as.Date()

# Load Old Data
old_data <- readRDS("Energy Analysis/data/cftc_ag_historical_clean.rds")
# Combine with New
updated_data <- c(setNames(list(ag_list), new_date), old_data)
# Save Combined
saveRDS(updated_data, "Energy Analysis/data/cftc_ag_dynamically_updated.rds")
