...
other_page <- read_html("https://www.cftc.gov/dea/options/other_lof.htm")

# Get preformatted text
other_preformatted <- other_page |>
  html_nodes("pre") |>
  html_text()
...
# Find Date from
new_date <- other_list[[1]][[1]][2] |>
  pull() |>
  as.Date()

# Load Old Data
old_data <- readRDS("Energy Analysis/data/cftc_other_historical_clean.rds")
# Combine with New
updated_data <- c(setNames(list(other_list), new_date), old_data)
# Save Combined
saveRDS(updated_data, "Energy Analysis/data/cftc_other_dynamically_updated.rds")
