...
nat_gas_page <- read_html("https://www.cftc.gov/dea/options/nat_gas_lof.htm")

# Get preformatted text
nat_gas_preformatted <- nat_gas_page |>
  html_nodes("pre") |>
  html_text()
...
# Find Date from
new_date <- nat_gas_list[[1]][[1]][2] |>
  pull() |>
  as.Date()

# Load Old Data
old_data <- readRDS("Energy Analysis/data/cftc_nat_gas_historical_clean.rds")
# Combine with New
updated_data <- c(setNames(list(nat_gas_list), new_date), old_data)
# Save Combined
saveRDS(updated_data, "Energy Analysis/data/cftc_nat_gas_dynamically_updated.rds")
